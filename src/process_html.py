...
def scrap_data(html):
    ...
    for grade in grade_lines:
        ...
        if "--" in grade:
            ...
            # Find top and bottom of fraction
            grade_split = (float(grade.split("/")[0]), float(grade.split("/")[1]))
...
def gen_worksheets(worksheets, path):
    ...
    for filename in worksheets:
        ...
        for grade, total in final_grade:
            ...
            worksheet_obj[filename].write(
                row_grades,
                col + 5,
                "=(D{}/E{})".format(row_grades + 1, row_grades + 1),
                percent_fmt,
            )
            row_grades += 1
        ...
        worksheet_obj[filename].conditional_format(
            "E2:F2", {"type": "formula", "criteria": '=$F$2="A"', "format": pass_format}
        )

        worksheet_obj[filename].conditional_format(
            "E2:F2",
            {"type": "formula", "criteria": '=$F$2="B"', "format": moderate_format},
        )

        worksheet_obj[filename].conditional_format(
            "E2:F2",
            {"type": "formula", "criteria": '=$F$2="C"', "format": moderate_format},
        )

        worksheet_obj[filename].conditional_format(
            "E2:F2", {"type": "formula", "criteria": '=$F$2="F"', "format": fail_format}
        )
